...
const create = async (req, res, next) => {
  try {
    ...
    if (alreadyHave)
      return new ErrorHandler(
        409,
        `This email already taken. Please try with a different email`,
        [],
        res
      );
  ...
  try {
    ...
    if (email && password) {
      ...
      if (!user || !user.validatePassword(password))
        return new ErrorHandler(400, "Email or password is invalid", [], res);
...
const acceptInvitation = async (req, res, next) => {
  try {
    ...
    if (!invitation)
      return new ErrorHandler(400, "Invitation not found", [], res);
    else {
      ...
      const project = await Project.findOneAndUpdate(
        { _id: invitation.project, "members.user": invitation.user },
        {
          $set: { "members.$.role": 2 },
          $push: {
            history: { title: `${user.firstName} joined the project.` },
          },
        }
      );
      ...
      await Promise.all([
        User.updateOne(
          { _id: invitation.user },
          { $addToSet: { contacts: owner } }
        ),
        User.updateOne(
          { _id: owner },
          { $addToSet: { contacts: invitation.user } }
        ),
  ...
  try {
    ...
    if (!invitation) new ErrorHandler(400, "Invitation not found", [], res);
      ...
      await Promise.all([
        Project.updateOne(
          { _id: invitation.project, "members.user": invitation.user },
          { $pull: { members: { user: invitation.user } } }
        ),
        Invitation.deleteOne({ _id: invitationId }),
      ]);
      res.status(200).json({ ok: 1 });
...
const testGet = async (req, res, next) => {
  try {
    const data = await User.findOne({ email: "testemail@gmail.com" });
    res.json(data);
  } catch (e) {
    next(e);
  }
...
const sendcode = async (req, res, next) => {
  try {
    ...
    if (email && code) {
      const user = await User.findOne({ email });
      if (!user)
        return   res.status(200).json({status:"error",message:"email not found"} );
        var transporter = nodemailer.createTransport({
          host: 'smtp.gmail.com',
          port:465,
          secure:true,
          auth: {
            user: 'care@alshammam.com',
            pass: 'tqctnxzxtlreqfby'
          },
          tls: {
        ...
        var mailOptions = {
          from: 'care@alshammam.com',
          to: email,
          subject: 'Alshammam Tasks Reset Password',
        ...
        transporter.sendMail(mailOptions, function(error, info){
          if (error) {
            ...
            res.status(200).json({status:"success",message:"code has been sent successfully"});

          }
        });
    } else res.status(200).json({status:"error",message:"Missing Request Fields"} );
  } catch (e) {
    next(e);
  }
};
  ...
  try {
    ...
    if (email && password) {
      ...
      if (!user)
        ...
        finalUser.save().then(async (data)=>{
    ...
    } else return res.status(200).json({status:"error",message:"Missing required fields"})
  } catch (e) {
    next(e);
  }
};
...
router.post("/removeUser", auth.required, removeUser);
