...
const create = async (req, res, next) => {
  try {
    const {name,createName, members} = req.body;
    const membersData = (members || []).map(m => ({user: m, role: 3}));
    const project = new Project({name,createName, members: [{user: req.payload.id, role: 1}, ...membersData]});
    await project.save();
    if (members?.length) {
...
const update = async (req, res, next) => {
  try {
    const {name, image, archived,favorite, calendar} = req.body;
    const project = await Project.findOneAndUpdate(
      {_id: req.params.id},
      {$set: getNotNullFields({name, image, archived,favorite, calendar})},
      {new: true}
      );
    if (archived !== undefined || archived)
      ...
      await Project.updateOne({_id: project}, {$push: {history: {title: `The project has ${favorite ? 'favorite' : 'unfavorite'}.`}}});
    res.status(200).json(project);
  } catch (e) {
    next(e);
  }
