...
const update = async (req, res, next) => {
  try {
    ...
    let todo = await Todo.findOneAndUpdate({ _id: req.params.id }, { $set: getNotNullFields({ text, checked }) }, { new: true });

    if (board) {
      todo = await Todo.findOneAndUpdate({ _id: req.params.id }, { $set: { board: board } }, { new: true });
    }

    if (board == 4) {
      todo = await Todo.findOneAndUpdate({ _id: req.params.id }, { $set: { checked: true } }, { new: true });
    } else {
      todo = await Todo.findOneAndUpdate({ _id: req.params.id }, { $set: { checked: false } }, { new: true });
    }
    res.status(200).json(todo);
  } catch (e) {
    next(e);
